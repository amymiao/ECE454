...
pthread_mutex_t mutex_list[16384]; //size of lists correspond to entries in hash table - each entry is the head of a list
...
/* randtrack_list_lock.cc function 
 * This function will find out which hash index the sample uses
 * and will lock that specific hash index 
 */
#ifdef LIST_LOCK
...
/* randtrack_list_lock.cc function 
 * This function will find out which hash index the sample uses
 * and will unlock that specific hash index 
 */
template<class Ele, class Keytype> 
void
